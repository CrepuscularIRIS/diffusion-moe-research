...
> **⚠ ROSTER SUPERSEDED 2026-07-13:** codex (GPT-5.5) REMOVED from the panel → forensic lane for
> /autopsy+/abduce only; panel = 5 families multi-instance. Authority = `moa/router-protocol.md`.
>
> **⚠ HISTORICAL DESIGN DOC.** The runtime authority is `moa/router-protocol.md` (differentiated per-advisor
> prompts — NOT the same-packet hand-off described in §3 below) + `.claude/CLAUDE.md` §1. Engine references
> below (Codex Stop-gate, `Pro 扩展`, DeepResearch, `agy:review`) describe the 2026-07-04 stack, superseded by
> the context-free GPT-5.6 review lane. Kept for design rationale only.
>
> **Status: ✅ IMPLEMENTED + PROVEN (2026-07-05; runtime roster updated 2026-07-07)** — scripts in `moa/`
> (`moa_ask.sh` · `moa_panel.sh` · `router-protocol.md`), wired into `manual §1`. Current default runtime
> panel = Gemini 3.1 Pro (`agy`) + GPT-5.5 (`codex`) + Opus 4.6 (Claude Code research subagent; bash
> compatibility via `claude -p --model claude-opus-4-6`) + MiMo V2.5 Pro + DeepSeek V4 Pro + Qwen3.7 Plus
> (`qwen3.7-plus`, not max). The last three use the same OpenCode gateway URL/key.
...
> judge/aggregator**. Q&A mode is chosen deliberately for the OpenCode gateway lanes; Opus 4.6 is a Claude Code
> research subagent, while Sonnet 4.6 is reserved for development execution.
...
| **Qwen3.7 Plus** | OpenCode gateway (`OpenCode-APIKEY1`), model `qwen3.7-plus` | orthogonal Chinese/technical reasoning lane | MED-HIGH | core gateway advisor |
| **Opus 4.6** | Claude Code `Agent` research subagent; bash compatibility `claude -p --model claude-opus-4-6` | Claude research-Q&A view (stronger than Sonnet for ideation) | **LOW** — same family as the judge | supplemental |
...
> judge's family** ⇒ treat it as an extra *sample*, not an independent view. The runtime panel keeps the old
> `agy`/`codex`/Claude-Code-`opus46` lanes and adds the gateway trio MiMo · DeepSeek · Qwen Plus. **★ ROLE SPLIT: Sonnet 4.6 is NOT a research advisor — it is the
> DEVELOPMENT executor** (native `Agent`, `model:"sonnet"`). DEV = Sonnet 4.6 + Opus 4.8 (orchestrator); the
> GPT-5.5 Codex Stop-gate is the intended diff reviewer. The Claude *research-Q&A* slot = Opus 4.6.
...
- **Native ClaudeCode subagents:** Opus 4.6 = research advisor `Agent`; Sonnet 4.6 = development executor
  `Agent` with `model:"sonnet"`. Subagents target a specific model ID directly, so Claude-family advisors need
  NO gateway. Opus 4.8 orchestrates + aggregates.
- **API advisors (OpenCode gateway):** MiMo V2.5 Pro + DeepSeek V4 Pro + Qwen3.7 Plus — POST to `$OpenCode-URL` with
...
2. Build the **dispute map**:
   - **CONSENSUS** — what ≥2 *independent* advisors converge on → high-confidence. *Weight the correlation:*
     cross-family + gateway agreement ≫ Claude-family agreement alone.
...
- **DO NOT use for:**
  - **Factual grounding / occupancy / prior-art** — the panel runs Q&A/UNGROUNDED (only the gateway lanes are
    ...
    tool-enabled paths (`agy research`, web search; Playwright→Pro DeepResearch DROPPED 2026-07-06, Cloudflare-blocked).
  - **The claim boundary (`/adversary` CLAIM_STANDS)** — stays the disciplined **independent** check
    (verified Codex Stop-gate verdict · `agy:review` Gemini · human). The panel is a **PROPOSER**; a Claude-heavy panel (Sonnet)
...
1. Query the OpenCode gateway `/models` (apikey1) → record the **exact model IDs** for DeepSeek V4 Pro,
   MiMo V2.5 Pro, and Qwen3.7 Plus.
2. Confirm `agy "ping"` returns a Gemini 3.1 Pro answer; confirm `codex exec "ping"` returns synchronously;
   confirm `claude -p --model claude-opus-4-6` returns for the Opus advisor compatibility path.
3. **Smoke test:** one real `/forge`-style question through all 6 advisors + the Opus reconciliation; eyeball
   the dispute map for genuine heterogeneity (discount Claude-family correlation).
4. **Degradation test:** kill one advisor mid-run; confirm Opus proceeds with the rest.

## 8. Cost / latency discipline
Per panel = 6 advisor calls (parallel, with Opus4.6 as Claude Code subagent/compat lane) + 1 Opus synthesis.
Acceptable on high-value ideation; **not per-turn.**
...
## 9. Integration point (DONE — implemented + proven 2026-07-05; the `moa/moa_*.sh` scripts are live)
When approved: the panel becomes the engine behind `/prospect` Mine + `/forge` candidate-generation — it
augments (does not replace) the current single-Pro dispatch with a 6-advisor fan-out + Opus reconcile. Build =
...
- **Claude Code Opus 4.6:** research advisor via native `Agent` subagent in Claude Code; bash compatibility via
  `claude -p --model claude-opus-4-6`. **Sonnet 4.6:** development executor subagent only, not a research advisor.
...
- **DeepSeek V4 Pro / MiMo V2.5 Pro / Qwen3.7 Plus:** API-only via OpenCode gateway (`OpenCode-APIKEY1`);
  runtime model IDs: `deepseek-v4-pro`, `mimo-v2.5-pro`, `qwen3.7-plus`.

## Current routing decision (2026-07-07)
Default research MoA = `gemini` (`agy`) + `gpt5` (`codex`) + `opus46` (Claude Code research subagent) +
`mimo` + `deepseek` + `qwen` (`qwen3.7-plus`, not max). Sonnet 4.6 is kept strictly as a development executor
subagent.
